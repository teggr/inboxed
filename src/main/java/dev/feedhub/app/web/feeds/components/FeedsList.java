...
public class FeedsList {
        ...
        div().with(
          ...
          table().withClasses(table, table_striped).with(
            ...
            tbody().with(
              ...
              each(feedConfigurations.getContent(), feed -> feedRow(feed, scheduledFetchFeedJobsByFeedId.get(feed.feedId())) )
  ...
  private static TrTag feedRow(FeedConfiguration feed, ScheduledJob scheduledFetchFeedJob) {
    return tr().with(

        td().with(text(feed.url().toString())),
        td().with(text(formatInstant(feed.createdDate()))),
        td().with(text( feed.schedule().toString() )),
        ...
        td().with(text( scheduledFetchFeedJob != null && scheduledFetchFeedJob.lastScheduledRun() != null ? formatInstant(scheduledFetchFeedJob.lastScheduledRun()) : "" )),
        td().with(text( scheduledFetchFeedJob != null && scheduledFetchFeedJob.lastScheduledRunResult() != null ? scheduledFetchFeedJob.lastScheduledRunResult().toString() : "" )),
